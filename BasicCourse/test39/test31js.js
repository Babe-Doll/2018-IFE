...
var sourceData = [{
	...
	window.onload=function(){
		if(history.state!==null){
			...
			c2.checked=false;
			c3.checked=false;
				for(var x in history.state.region){
					if(c1.value==history.state.region[x]){
						c1.checked=true;
						
					}else if(c2.value==history.state.region[x]){
						c2.checked=true;
					}
					else if(c3.value==history.state.region[x]){
						c3.checked=true;
					}else{
			...
			c5.checked=false;
			c6.checked=false;
				for(var p in history.state.product){
					if(c4.value==history.state.product[p]){
						c4.checked=true;
						
					}else if(c5.value==history.state.product[p]){
						c5.checked=true;
					}
					else if(c6.value==history.state.product[p]){
						c6.checked=true;
					}else{
			...
			table2.innerHTML="";
			buildtable2(history.state.region,history.state.product);
		}
		    create();
	}
	function backvalue(){
	...
	function addstate(obj1,obj2){
		if (!!(window.history && history.pushState)) {
   			var url={region:obj1,product:obj2};
			history.pushState(url,null,"?"+url.region+"-"+url.product);
		...
		window.addEventListener("popstate",function(e){
			...
			c6.checked=false;
				for(var p in history.state.product){
					...
					// else if(c4.checked=true&&c5.checked=true&&c6.checked=true){
					// 	checkall2.checked=true;
					// }
					else{
